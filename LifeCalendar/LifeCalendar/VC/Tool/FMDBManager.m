...
-(BOOL)createTableWithClassString:(NSString*)classStr dataBase:(FMDatabase*)db{
    ...
    NSString *createStr =[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@(modelID INTEGER NOT NULL PRIMARY KEY %@)",tableName,TableStr];
...
- (NSArray *)getPropertyAndType_OC:(Class)classname      //按oc的类型返回
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        // developer.apple.com/library/mac/documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtTypeEncodings.html
        ...
        else if([type_flag isEqualToString:@"B"]) {
            //bool
            p_type = @"BOOL";
        }else if ([type_flag isEqualToString:@"q"]){
            p_type = @"long";
        }
        else {
            p_type = @"int";
...
- (NSString *)getSqliteTableStr:(Class)classname
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        else if ([tindex isEqualToString:@"b"]) {
            type = @"INTEGER";
        }else{
            type = @"TEXT";
        }
...
-(NSArray *)queryTable:(Class)modelClass QueryString:(NSString *)sql{
    ...
    [_operation inDatabase:^(FMDatabase *db) {
        [db executeStatements:sql withResultBlock:^int(NSDictionary *resultsDictionary) {
            DMModel *model = [[modelClass alloc]init];
...
-(BOOL)saveDataWithModel:(DMModel *)model{
    ...
    [_operation inDatabase:^(FMDatabase *db) {
        [self createTableWithClassString:NSStringFromClass(model.class) dataBase:db];
...
-(BOOL)inserSqlWithModel:(DMModel*)model clomnNameArr:(NSArray*)clomnArr dataBase:(FMDatabase*)db{
...
-(BOOL)deleteDataWithModel:(DMModel *)model{
    ...
    [_operation inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(model.class);
        NSString *sql = [NSString stringWithFormat:@"DELETE FROM %@ where modelID = '%d'",tableName,model.modelID];
...
-(BOOL)hasTheModel:(DMModel *)model{
    NSString *tableName = NSStringFromClass(model.class);
    NSInteger modelIDValue = model.modelID;
    __block int count = 0;
    if (modelIDValue) {
        NSString *sql = [NSString stringWithFormat:@"SELECT COUNT(*) from %@ where modelID ='%ld'",tableName,modelIDValue];
